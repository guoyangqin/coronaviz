...
import numpy as np
from config.country_demographics import country_demographics
...
for i in range(top_num):
    ...
    sub_infection_rate[infection_rate_country] = (sub_num_cases2[infection_rate_country].cumsum()
                                                  / (country_demographics[infection_rate_country]['pop_million'] * 1e6)
                                                  ).values
...
# Save figures
figure1.savefig('figures/num_cases.pdf', transparent=True)
figure2.savefig('figures/num_deaths.pdf', transparent=True)
figure3.savefig('figures/fatality_rate.pdf', transparent=True)
figure4.savefig('figures/infection_rate.pdf', transparent=True)
