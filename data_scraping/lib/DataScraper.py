...
class DataScraper:
    ...
    def get_data_from_worldmeters(self):
        ...
        elem_text = [e for e in elem_text_list if 'coronavirus-cases-linear' in e][0]

        # Clean text
        text = elem_text.replace('\n', '').replace('\'', '"').replace(' ', '')
        text = [t for t in text.split('Highcharts.chart') if 'coronavirus-cases-linear' in t][0]
